...
from src import TheTVDBHandler

class TestAuthentication(unittest.TestCase):

    def test_newAuthentication(self):
        ...
        # Need to create a mock object to represent the urllib2 client.
        # With the help of `spec` argument, the mock object will have same interface.
        mock_http_client = unittest.mock.Mock(spec=urllib)
        # and replace the explicit dependency
        handler._http_client = mock_http_client

        handler._checkAuthentication()
        mock_http_client.Request.assert_called_with(theTVDBHandler_config["urls"]["authentication_login"])

    #     self.assertEqual('foo'.upper(), 'FOO')

    # def test_refreshingAuthentication(self):
    #     self.assertTrue('FOO'.isupper())
    #     self.assertFalse('Foo'.isupper())

    # def test_stillValidAuthentication(self):
    #     s = 'hello world'
